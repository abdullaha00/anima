...
import { Citations, DRAFT_ONLY_LINE, FAMILY_DRAFT_LINE } from "@/components/review/Citations";
 ...
 */
export function MeetingBriefing({ meeting }: { meeting: Stage2Assessment["meeting"] }) {
  return (
    <Panel title="Meeting briefing, from the record review">
      <div className="flex flex-col gap-4">
        ...
        <Field label="Format and accessibility">{meeting.formatAndAccessibility}</Field>
        ...
        <p className="border-t border-line pt-3 text-[12px] leading-5 text-faint">
          A prompt for the clinician who runs the meeting, not a decision. Nothing here is posted to the thread.
        </p>
      </div>
    </Panel>
  );
...
export function DraftCommunications({
  ...
  return (
    <Panel title={family ? "Draft for the family, from the record review" : "Draft communications"}>
      <ul className="flex flex-col divide-y divide-line">
        {communications.map((c, i) => {
          // The family channel's content rules apply to a draft as they would to an entry.
          const guard = family ? checkFamilyContent(c.draft) : { ok: true as const };
          return (
          <li key={i} className="flex flex-col gap-3 py-4 first:pt-0 last:pb-0">
            <p className="text-[13px] font-medium leading-5 text-secondary">{family ? FAMILY_DRAFT_LINE : DRAFT_ONLY_LINE}</p>
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Audience">{c.audience}</Field>
              <Field label="Channel">{c.channel}</Field>
            </div>
            {!guard.ok ? (
              <Notice kind="refuse" title="Would be refused in the family channel">
                {guard.reason}: &ldquo;{guard.matched}&rdquo;. Rewrite before any of this reaches the family.
              </Notice>
            ) : null}
            <blockquote className="prose-clinical font-voice text-[17px] leading-[1.45] text-ink">{c.draft}</blockquote>
            {c.cautions.length ? (
              <div className="flex flex-col gap-1">
