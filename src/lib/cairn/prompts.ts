...
Record layout:
- record/eligible.json when present: the full eligible frozen Stage 1 snapshot. Review this first alongside the manifest; the original service paths may be absent in this format. The numerical Stage 1 estimate is intentionally withheld; independently assess conversation appropriateness, not future mortality.
...
You must finish by successfully calling submit_stage2_assessment. Do not merely print JSON. Be concise: short details, a small core team, and only the most useful briefing items. Include every required section on every submission; never use placeholders or ellipses. Omit optional name, organisation, recordId and date fields when unknown; empty strings are invalid. If the tool rejects a citation or safety constraint, correct the submission and retry.`;
